          ...
          <>
            {ranking.length === 0 ? (
              ...
              <div className="bg-dark-card rounded-xl border border-dark-border overflow-hidden">
                {ranking.map((r, i) => {
                  ...
                  return (
                    <div key={r.user_id} className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? 'border-t border-dark-border' : ''}`}>
                      <span className={`w-7 text-center font-mono text-sm ${
                        ...
                        {r.position <= 3 ? ['', '1', '2', '3'][r.position] : r.position}
                      </span>
            ...
            {userPosition && (
              <div className="bg-dark-card border border-gold/30 rounded-xl px-4 py-3 flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-sm font-mono text-gold">{userPosition.elo} ELO</p>
                ...
                <div key={week} className="bg-dark-card rounded-xl border border-dark-border p-4">
                  ...
                  {entries.map(e => (
                    <div key={e.id} className="flex items-center gap-2 py-1">
                      <span className="text-sm">{['', '1', '2', '3'][e.position]}</span>
                      <span className="text-sm flex-1">{e.cc_profiles?.display_name || 'Usuario'}</span>
                      <span className="text-xs font-mono text-gold">{e.elo} ELO</span>
                    </div>
                  ))}
                </div>
              ))}
