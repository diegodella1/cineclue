...
export function useDaily() {
  ...
  const [result, setResult] = useState(null) // { guessed, points, clue_revealed }
  ...
  const loadDaily = useCallback(async () => {
    setLoading(true)
    const { data, error } = await supabase.rpc('cc_get_today_daily', {
  ...
  const completeDaily = async (r) => {
    ...
    await supabase.rpc('cc_complete_game', {
      p_user_id: user.id,
      p_mode: 'daily',
